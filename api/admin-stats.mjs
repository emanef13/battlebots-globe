...
const QUERIES = {
  ...
  top_bots: `SELECT properties.bot, count() FROM events
             WHERE event = 'bot_selected' AND timestamp > now() - INTERVAL 7 DAY
  ...
  fights: `SELECT count(), count(DISTINCT person_id) FROM events
           WHERE event = 'fight_mode' AND timestamp > now() - INTERVAL 7 DAY`,
  searches: `SELECT properties.query, count() FROM events
             WHERE event = 'search_select' AND timestamp > now() - INTERVAL 7 DAY
